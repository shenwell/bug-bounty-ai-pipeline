...
from vuln_taxonomy import (  # noqa: E402
    GATE_TO_CANONICAL,
    build_canonical_set,
    gate_to_canonical,
    resolve_matrix_key,
...
CANONICAL_VULN_CLASSES = build_canonical_set()
...
PATH_SIGNAL_MAP: list[tuple[re.Pattern[str], list[str], str]] = [
    ...
    (
        re.compile(r"/(s3|blob|storage|bucket|assets\.amazonaws)"),
        ["cloud-misconfig", "ssrf", "info-disclosure"],
        "cloud storage signal",
    ),
    (
        re.compile(r"\.(s3\.amazonaws|blob\.core\.windows|storage\.googleapis)"),
        ["cloud-misconfig", "info-disclosure"],
        "cloud host signal",
    ),
]
...
def _canonicalize_vuln_label(label: str) -> str | None:
    ...
    if "lfi" in text or "path traversal" in text or "directory traversal" in text:
        return "lfi"
    if "privilege escalation" in text or "privilege-escalation" in text.replace(" ", "-"):
        return "privilege-escalation"
    if "subdomain takeover" in text or "subdomain-takeover" in text.replace(" ", "-"):
        return "subdomain-takeover"
    if "llm" in text or "llm-ai" in text.replace(" ", "-"):
        return "llm-ai"
    if "cloud misconfig" in text or "cloud-misconfig" in text.replace(" ", "-"):
        return "cloud-misconfig"
    if "mobile" in text and "api" in text:
        return "mobile-api"
    if "web3" in text or "smart contract" in text:
        return "web3"
    if "cache deception" in text or "cache-deception" in text.replace(" ", "-"):
        return "cache-deception"
    if "header injection" in text or "header-injection" in text.replace(" ", "-"):
        return "header-injection"
    if "h2 desync" in text or "h2-desync" in text.replace(" ", "-"):
        return "h2-desync"
    if "method confusion" in text or "method-confusion" in text.replace(" ", "-"):
        return "method-confusion"
    # Direct gate slot labels (coverage-xss-reflected, etc.)
    if text.replace(" ", "-") in GATE_TO_CANONICAL:
        return gate_to_canonical(text.replace(" ", "-"))
    return None
...
def _extract_exhausted_classes(target: str, brain_dir: Path | None = None) -> set[str]:
    ...
    if target_file.exists():
        ...
        for match in re.findall(r"coverage-([a-z0-9_-]+)", content):
            ...
            if canon:
                exhausted.add(canon)
            elif match in GATE_TO_CANONICAL:
                exhausted.add(gate_to_canonical(match))
...
def recommend_vuln_classes(
    ...
    telemetry: dict | None = None,
    limit: int = 8,
    threat_boosts: dict[str, float] | None = None,
    slice_boosts: dict[str, float] | None = None,
    intent_boosts: dict[str, float] | None = None,
) -> list[dict]:
    ...
    for cls, boost in (threat_boosts or {}).items():
        canon = _canonicalize_vuln_label(cls) or cls
        if canon in scores:
            scores[canon] += boost
            reasons[canon].append(f"threat model boost (+{boost:.1f})")

    for cls, boost in (slice_boosts or {}).items():
        canon = _canonicalize_vuln_label(cls) or cls
        if canon in scores:
            scores[canon] += boost
            reasons[canon].append(f"focus slice boost (+{boost:.1f})")

    for cls, boost in (intent_boosts or {}).items():
        canon = _canonicalize_vuln_label(cls) or cls
        if canon in scores:
            scores[canon] += boost
            reasons[canon].append(f"session-intent selected (+{boost:.1f})")
        else:
            scores[canon] = boost
            reasons[canon].append(f"session-intent selected (+{boost:.1f})")
...
def _load_traffic_hits(path: Path | None) -> dict[str, int]:
    ...
    if isinstance(data, list):
        for row in data:
            if isinstance(row, str):
                hits[row.lower()] = hits.get(row.lower(), 0) + 1
            elif isinstance(row, dict):
                ep = str(row.get("endpoint") or row.get("url") or "").lower()
                if ep:
                    hits[ep] = int(row.get("request_count", 1) or 1)
    elif isinstance(data, dict):
        for ep, count in data.items():
            hits[str(ep).lower()] = int(count or 1)
    return hits


def _resolve_class_boosts(
    root: Path,
    target: str,
    focus_areas_path_arg: str | None,
    slice_id: str | None,
    session_intent_path_arg: str | None = None,
) -> tuple[dict[str, float], dict[str, float], dict[str, float]]:
    threat_boosts: dict[str, float] = {}
    slice_boosts: dict[str, float] = {}
    intent_boosts: dict[str, float] = {}
    try:
        from threat_model import load_threat_model, ranked_class_boosts, threat_model_path
        from focus_areas import get_slice, load_focus_areas, slice_class_boosts
        ...
        threat_boosts = ranked_class_boosts(tm)
        fa_path = Path(focus_areas_path_arg) if focus_areas_path_arg else root / "brain" / "focus-areas" / f"{target.replace('.', '-')}.json"
        if not fa_path.exists():
            from threat_model import slugify_target

            fa_path = root / "brain" / "focus-areas" / f"{slugify_target(target)}.json"
        fa = load_focus_areas(fa_path)
        if fa and slice_id:
            slice_row = get_slice(fa, slice_id)
            slice_boosts = slice_class_boosts(slice_row)
        from session_intent import session_intent_boosts

        intent_boosts = session_intent_boosts(root, target)
    except Exception:
        pass
    return threat_boosts, slice_boosts, intent_boosts
...
def show_recommended_classes(
    ...
    focus_areas_path: str | None = None,
    ...
    threat_boosts, slice_boosts, intent_boosts = _resolve_class_boosts(
        root or Path("."), target, focus_areas_path, slice_id
    )
    ranked = recommend_vuln_classes(
        ...
        telemetry=telemetry,
        limit=limit,
        threat_boosts=threat_boosts,
        slice_boosts=slice_boosts,
...
def score_surface_endpoint(
    endpoint: str,
    tech_stack: str = "",
    traffic_hits: dict[str, int] | None = None,
    fuzz_signals: list[dict] | None = None,
    fuzz_boost_killed: set[str] | None = None,
) -> dict[str, object]:
    ...
    if traffic_hits:
        count = traffic_hits.get(endpoint.strip().lower(), 0)
        if count > 0:
            boost = min(15.0, 5.0 + count * 2.0)
            score += boost
            reasons.append(f"traffic-observed x{count} (+{boost:.1f})")

    if fuzz_signals and (fuzz_boost_killed is None or lower not in fuzz_boost_killed):
        for sig in fuzz_signals:
            if sig.get("signal_tier") != "oracle+readback":
                continue
            sig_ep = str(sig.get("endpoint") or "").strip().lower()
            if sig_ep == lower or sig_ep.rstrip("/") == lower.rstrip("/"):
                energy = float(sig.get("energy") or 1.0)
                boost = min(15.0, 8.0 + energy * 2.0)
                score += boost
                reasons.append(f"fuzz-signal oracle+readback (+{boost:.1f})")
                break
...
def rank_surface(
    ...
    for raw in endpoints:
        ...
        seen.add(ep)
        scored.append(
            score_surface_endpoint(
                ep,
                tech_stack=tech_stack,
                traffic_hits=traffic_hits,
                fuzz_signals=fuzz_signals,
                fuzz_boost_killed=fuzz_boost_killed,
            )
        )
...
def _load_ai_fingerprints(root: Path) -> dict:
    for rel in ("rules/ai-infra-fingerprints.json", "rules/ai-infra-fingerprints.yaml"):
        path = root / rel
        if not path.exists():
            continue
...
def _collect_recon_corpus(root: Path) -> str:
    parts: list[str] = []
    for pattern in (
        "recon/**/*.txt",
        "recon/**/*.json",
        "recon/**/*.md",
        "brain/threat-model/*.json",
        "brain/threat-model/*.md",
    ):
...
def detect_ai_infra(root: Path | None = None) -> dict:
    """Scan recon corpus for AI/MCP infra signals."""
    base = root or Path(".")
    fp = _load_ai_fingerprints(base)
    corpus = _collect_recon_corpus(base)
    matches: list[dict] = []
    boost_classes: dict[str, float] = {}
    ...
    for comp in fp.get("components") or []:
        ...
        for port in comp.get("ports") or []:
            if re.search(rf":{port}\b", corpus):
                hit_reasons.append(f"port:{port}")
        for sig in comp.get("path_signals") or []:
            if str(sig).lower() in corpus:
                hit_reasons.append(f"path:{sig}")
        for sig in comp.get("header_signals") or []:
            if str(sig).lower() in corpus:
                hit_reasons.append(f"header:{sig}")
        if hit_reasons:
            cls = str(comp.get("boost_class", "llm-ai"))
            boost_classes[cls] = boost_classes.get(cls, 0) + 2.0
            matches.append(
                {
                    "component": comp.get("id"),
                    "name": comp.get("name"),
                    "reasons": hit_reasons,
                    "cve_refs": comp.get("cve_refs") or [],
                    "boost_class": cls,
                }
            )

    for group_key in ("mcp_signals", "chat_ui_signals"):
        for row in fp.get(group_key) or []:
            if not isinstance(row, dict):
                continue
            pat = str(row.get("pattern", "")).lower()
            if pat and pat in corpus:
                cls = str(row.get("boost_class", "llm-ai"))
                boost_classes[cls] = boost_classes.get(cls, 0) + 1.5
                matches.append(
                    {
                        "component": group_key,
                        "pattern": pat,
    ...
    return {
        "ai_surface": len(matches) > 0,
        "matches": matches,
        "boost_classes": boost_classes,
    }


def show_detect_ai_infra(root: Path, output: Path | None = None) -> int:
    result = detect_ai_infra(root)
...
def _fuzz_boost_killed_from_brain(brain_dir: Path) -> set[str]:
    """Endpoints whose fuzz boost decayed after hunter KILL."""
    killed: set[str] = set()
    ...
    for path in brain_dir.rglob("*.md"):
        ...
        for line in text.splitlines():
            low = line.lower()
            if "kill" in low and "fuzz-boost-applied:" in low:
                part = line.split("fuzz-boost-applied:", 1)[-1].strip().split()[0]
                killed.add(part.lower())
    return killed
...
def show_ranked_surface(
    ...
    fuzz_signals_file: Path | None = None,
    root: Path | None = None,
) -> int:
    if not endpoints_file.exists():
        print(f"Endpoints file not found: {endpoints_file}")
        return 1
    traffic_hits = _load_traffic_hits(traffic_file)
    fuzz_signals: list[dict] = []
    if fuzz_signals_file and fuzz_signals_file.exists():
        try:
            import json as _json

            data = _json.loads(fuzz_signals_file.read_text(encoding="utf-8"))
            fuzz_signals = data if isinstance(data, list) else []
        except Exception:
            fuzz_signals = []
    ...
    ranked = rank_surface(
        _load_endpoints_file(endpoints_file),
        tech_stack=tech_stack,
        limit=limit,
        traffic_hits=traffic_hits,
        fuzz_signals=fuzz_signals,
...
def _default_attack_matrix() -> dict[str, dict[str, list[str]]]:
    ...
    return {
        ...
        "auth-bypass": {
            "vectors": ["session-cookie", "jwt-header", "password-reset", "mfa-step", "api-key"],
            "encodings": ["raw", "url", "base64", "unicode-normalize"],
            "bypasses": ["method-override", "path-normalize", "role-claim", "host-header", "cache-poison-session"],
        },
        "cloud-misconfig": {
            "vectors": ["s3-list", "s3-read", "iam-policy", "k8s-rbac", "azure-blob"],
            "encodings": ["raw", "url", "signed-url"],
            "bypasses": ["public-acl", "bucket-policy", "anonymous-read", "metadata-ssrf-chain"],
        },
        "mobile-api": {
            "vectors": ["apk-endpoint", "deep-link", "webview-bridge", "push-token", "cert-pin-bypass"],
            "encodings": ["raw", "url", "base64"],
            "bypasses": ["hardcoded-key", "ssl-pinning-off", "intent-filter", "exported-activity"],
        },
        "prototype-pollution": {
            "vectors": ["json-merge", "query-string", "cookie-json", "postmessage"],
            "encodings": ["raw", "url", "constructor-chain"],
            "bypasses": ["__proto__", "constructor.prototype", "nested-merge", "gadget-rce"],
        },
        "xxe": {
            "vectors": ["soap", "svg-upload", "docx", "rss", "saml-assertion"],
            "encodings": ["raw", "url", "utf16", "parameter-entity"],
            "bypasses": ["external-entity", "xinclude", "php-filter", "oob-dtd", "svg-onload"],
        },
    }
...
def build_attack_matrix(
    ...
    key = resolve_matrix_key(vuln_class.strip().lower())
    if key not in matrix:
        return []
    cfg = matrix[key]
    combos: list[dict[str, str]] = []
    requested = vuln_class.strip().lower()
    for vector in cfg["vectors"]:
        for encoding in cfg["encodings"]:
            for bypass in cfg["bypasses"]:
                combos.append(
                    {
                        "vuln_class": requested,
                        "vector": vector,
                        "encoding": encoding,
                        "bypass": bypass,
                        "label": f"{requested}:{vector}:{encoding}:{bypass}",
                    }
                )
                if len(combos) >= limit:
                    return combos
...
def main() -> None:
    ...
    classes_p.add_argument("--brain-dir", default="brain")
    ...
    classes_p.add_argument("--focus-areas", default=None, help="Path to brain/focus-areas/<target>.json")
    classes_p.add_argument("--slice", default=None, help="Focus slice id for class boost")
    classes_p.add_argument("--root", default=".", help="Workspace root for threat/focus/session-intent artifacts")
    ...
    rank_p.add_argument(
        "--traffic-file",
        default=None,
        help="JSON file of observed endpoints (recon/traffic-exercised.json)",
    )
    rank_p.add_argument(
        "--fuzz-signals-file",
        default=None,
        help="JSON file of oracle+readback signals (recon/fuzz-signals.json)",
    )
    rank_p.add_argument("--root", default=".", help="Workspace root for brain fuzz-boost decay")
    ai_p = sub.add_parser(
        "detect-ai-infra",
        help="Detect AI/MCP infra signals in recon corpus; boost llm-ai class",
    )
    ai_p.add_argument("--root", default=".")
    ai_p.add_argument("--output", default=None, help="Write JSON to path (default stdout)")
    ...
    budget_p.add_argument("--brain-dir", default="brain")
    budget_p.add_argument("--telemetry-path", default=".autonomy-telemetry.json")
    budget_p.add_argument("--focus-areas", default=None)
    budget_p.add_argument("--slice", default=None)
    budget_p.add_argument("--root", default=".")
    ...
    chain_p.add_argument(
        "--capability-file",
        default="brain/patterns/capability-graph.json",
    )
    chain_p.add_argument(
        "--output",
        default=None,
    ...
    if args.command == "analyze":
        ...
        output = Path(args.output) if args.output else None
        traffic = Path(args.traffic_file) if args.traffic_file else None
        fuzz_file = Path(args.fuzz_signals_file) if args.fuzz_signals_file else None
        sys.exit(
            show_ranked_surface(
                ...
                fuzz_signals_file=fuzz_file,
                root=Path(args.root),
            )
        )
    elif args.command == "detect-ai-infra":
        output = Path(args.output) if args.output else None
        sys.exit(show_detect_ai_infra(Path(args.root), output=output))
