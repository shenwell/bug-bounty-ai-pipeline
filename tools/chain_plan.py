...
def _read_structured_findings(root: Path | None = None) -> list[dict]:
    """Load finding_record JSON files from evidence/**/findings/."""
    base = root or Path(".")
    out: list[dict] = []
    ...
    for path in evidence.glob("**/findings/*.json"):
        ...
        if witness.get("ok") is True or validator.get("decision") == "PASS":
            if entry.get("status") not in {"killed"}:
                out.append(entry)
        elif entry.get("status") in {"confirmed", "potential"}:
            out.append(entry)
    return out


def _read_findings(root: Path | None = None) -> list[dict]:
    """Return list of finding dicts — structured evidence JSON first, then legacy findings.json."""
    structured = _read_structured_findings(root)
    if structured:
        return structured
...
def _classify(finding: dict) -> str | None:
    haystack = " ".join(
        ...
        for k in ("title", "vuln_type", "weakness", "type", "category", "class")
    )
