...
from tools.vuln_taxonomy import (
    REQUIRED_CLASSES,
    SCOPE_SKIPPABLE_CLASSES,
...
WORKFLOW_DISRUPTION_RE = re.compile(
    r"workflow-disruption\s*:\s*(?P<path>\S+)\s+axes\s*:\s*(?P<axes>[\w,-]+)\s+evidence\s*:\s*(?P<evidence>\S+)",
    re.IGNORECASE,
)
SIGNAL_FUZZ_RE = re.compile(
    r"signal-fuzz\s*:\s*(?P<endpoint>\S+)\s+tier\s*:\s*oracle\+readback",
    re.IGNORECASE,
)
...
def _validate_coverage_json(vuln_class: str, record: dict, root: Path) -> list[str]:
    ...
    try:
        from tools.session_bridge import injection_exhaustion_requires_session, role_exhaustion_requires_multi_session

        allowed, waf_reason = injection_exhaustion_requires_session(root, vuln_class)
        if not allowed:
            issues.append(waf_reason)
        role_ok, role_reason = role_exhaustion_requires_multi_session(root, vuln_class)
        if not role_ok:
            issues.append(role_reason)
    except Exception:
        pass
...
def _target_file_for(root: Path, brain_meta: dict[str, dict[str, str]], target: str) -> tuple[Path | None, str, str]:
    ...
    path = root / "brain" / "targets" / f"{slug}.md"
    if path.exists():
...
def _validate_workflow_disruption(
    target: str,
    content: str,
    root: Path,
    result: GateResult,
) -> None:
    """v1.5: business-logic / race exhaustion on write surface needs disruption marker."""
    slug = slugify_target(target)
    surface_dirs = _evidence_surface_dirs(root, target, slug)
    surface = _surface_summary(surface_dirs)
    if not surface.get("write_surface"):
        return

    for vuln_class in ("business-logic", "race-condition"):
        lines = _coverage_lines(content, vuln_class)
        if not lines:
            continue
        joined = "\n".join(lines).lower()
        if not any(tok in joined for tok in ("exhaust", "no-signal", "blocked", "remaining: 0")):
            continue
        if WORKFLOW_DISRUPTION_RE.search(content):
            continue
        result.add(
            "missing-workflow-disruption",
            (
                f"{vuln_class} exhaustion on write surface lacks workflow-disruption marker; "
                "record `workflow-disruption:<path> axes:skip-step,double-submit evidence:<file.jsonl>`"
            ),
            target,
        )
...
def _validate_traffic_informed(root: Path, brain: dict[str, dict[str, str]], result: GateResult) -> None:
    from tools.traffic_informed import traffic_skip_recorded, validate_traffic_informed

    brain_content = _all_brain_content(brain)
    if traffic_skip_recorded(brain_content):
        return
    ok, reason = validate_traffic_informed(root, brain_content)
    if not ok:
        result.add("traffic-informed", reason)


def _validate_session_intent(root: Path, target_arg: str, result: GateResult) -> None:
    """Require session-intent.json when auth context exists (traffic or role sessions)."""
    from tools.session_intent import validate_session_intent
    from tools.session_bridge import active_role_session_count, session_exists

    traffic = root / "recon" / "traffic-exercised.json"
    has_auth_context = (
        (traffic.exists() and traffic.stat().st_size > 0)
        or session_exists(root)
        or active_role_session_count(root) > 0
    )
    if not has_auth_context:
        return
    target = normalize_target(target_arg)
    if not target and result.p1_targets:
        target = result.p1_targets[0]
    if not target:
        return
    ok, issues = validate_session_intent(root, target)
    if not ok:
        for issue in issues:
            result.add("session-intent", issue, target)
...
def _validate_confirmed_findings(result: GateResult, target: str, content: str, root: Path) -> None:
    from tools.finding_record import gates_complete_for_confirm, load_finding, parse_json_ref_from_brain

    for line in re.findall(r"^.*\[\s*CONFIRMED\s*\].*$", content, flags=re.MULTILINE | re.IGNORECASE):
        json_path = parse_json_ref_from_brain(line)
        if json_path:
            full = root / json_path
            record = load_finding(full)
            if not record:
                result.add("finding-json-missing", f"confirmed finding JSON not found: {json_path}", target)
                continue
            ok, missing = gates_complete_for_confirm(record)
            if not ok:
                result.add(
                    "finding-gates-incomplete",
                    f"finding {record.get('id')} missing gates: {', '.join(missing)}",
                    target,
                )
            continue
        lower = line.lower()
    ...
    # Scan evidence/*/findings/*.json with status confirmed
    for path in root.glob("evidence/*/findings/*.json"):
        try:
        ...
        if not ok:
            result.add(
                "finding-gates-incomplete",
                f"{path}: confirmed JSON missing gates: {', '.join(missing)}",
                target,
            )
...
def evaluate_project(
    ...
    classes_to_check: list[str] = list(REQUIRED_CLASSES)
    if wave_only and slice_id:
        try:
            from tools.focus_areas import focus_areas_path, get_slice, load_focus_areas

            seed = target_seed or (result.p1_targets[0] if result.p1_targets else "")
            fa = load_focus_areas(focus_areas_path(root, seed))
            slice_row = get_slice(fa, slice_id) if fa else None
            if slice_row:
                allowed_hosts = {h.lower() for h in (slice_row.get("hosts") or [])}
                result.p1_targets = [
                    h
                    for h in result.p1_targets
                    if h.lower() in allowed_hosts or any(ah in h.lower() for ah in allowed_hosts)
                ]
                slice_classes = [str(c) for c in (slice_row.get("priority_classes") or [])]
                if slice_classes:
                    classes_to_check = [c for c in REQUIRED_CLASSES if c in slice_classes]
        except Exception:
            pass
    ...
    for target in result.p1_targets:
        ...
        for vuln_class in classes_to_check:
        ...
        _validate_workflow_disruption(target, content, root, result)

        _validate_cross_region_inference(target, content, result)

        _validate_confirmed_findings(result, target, content, root)

    _validate_recon_depth(root, brain, result)
    _validate_traffic_informed(root, brain, result)
    _validate_session_intent(root, target_arg, result)

    pending = root / "brain" / "chain-pending.md"
    if not pending.exists():
        pending = root / "chain-pending.md"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--wave-only",
        action="store_true",
        help="Slice-scoped gate: only validate hosts/classes for current focus slice",
    )
    parser.add_argument("--slice-id", default="", help="Focus slice id for --wave-only mode")
    return parser
