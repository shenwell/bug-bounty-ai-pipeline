...
4. **PoC**: Self-contained file. Screenshots at each step. Video if multi-step.
   UI screenshots: full Chrome window + address bar via `PrintWindow` (not viewport-only).
   Workflow: isolated CDP Chrome → `capture_*_screenshots.py` → `tools/browser_screenshot.py verify`.
   See `rules/screenshots.md`. Never cite PNGs until verify exits 0.
...
- Submission package: `data/dossiers/<Dossier>/leads/<lead>/` (`policy/submit.md`, `pack_submit.py`)

## Optional Russian report locale

When `.cursor/rules/pentest-agents-report-style-ru.mdc` is enabled:

- Language: Russian for report prose and section titles
- Formatting: no bold (`**`) or decorative emphasis
- Title formula (Russian): `[уязвимость] в [компоненте] позволяет [влияние]`
- Rule file: `.cursor/rules/pentest-agents-report-style-ru.mdc`
