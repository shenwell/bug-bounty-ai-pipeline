...
6. Build a **depth matrix** before declaring a class exhausted:
   ...
   - Minimum 30 combinations on P1 surface (seed with `uv run python3 $tools/intel_engine.py matrix <class>`)
...
11. **Log negative evidence** (what failed and why) via `uv run python3 $tools/brain.py record <target> recon "coverage-<class>" "<details>"` so autopilot resume avoids repeating exhausted paths.
