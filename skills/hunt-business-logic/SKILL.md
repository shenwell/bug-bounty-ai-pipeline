...
## Privilege Escalation (PE) — gate class `privilege-escalation`

PE overlaps IDOR and business-logic; this section is the **permissions-first** playbook (Rule 22).

### Role matrix template

| Action / endpoint | unauth | userA | userB | admin | Expected |
|-------------------|--------|-------|-------|-------|----------|
| `GET /api/admin/users` | | | | | admin only |
| `POST /api/roles` | | | | | admin only |
| `PATCH /api/users/{id}/role` | | | | | self or admin |

Fill every row from UI walk + JS bundle + network replay. Any cell where low-priv succeeds = PE lead.

### High-signal PE patterns

1. **Hidden admin API** — button hidden in UI but `POST /api/admin/...` returns 200.
2. **Mass assignment** — `role`, `is_admin`, `tier`, `permissions[]` in JSON body.
3. **JWT/session claim** — `role:admin` without signature break (pair with hunt-auth-bypass).
4. **GraphQL admin mutation** — dispatch graphql-audit + idor-hunter.
5. **SCIM / provisioning** — role set via `/scim/v2/Users` without tenant check.

### Kill signals

- "Admin can access admin panel" — not a bug.
- UI-only restriction with API 403 on all variants.
- PE without read-back from second session.

Dispatch `privilege-escalation` agent for gate; cross-link `hunt-idor` for object-level PE.

## Race Condition deep dive — gate class `race-condition`

Financial and quota endpoints only (Rule 11). Pair with coupon/billing sections above.

### Minimum test

```bash
# 20 parallel identical state-changing requests
seq 1 20 | xargs -P 20 -I {} curl -s -X POST "$URL" \
...
1. **Read-back** — GET order/balance/coupon-usage after burst.
2. **Idempotency** — repeat with `Idempotency-Key` header if API documents it.
3. **Ledger delta** — document before/after numeric state.

### Chain

Race → inflated balance → purchase paid feature → ATO via credit abuse.

Kill: all but one request rolled back; final state matches single-use invariant.
