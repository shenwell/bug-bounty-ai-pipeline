...
## Rule 32: «РЕЗУЛЬТАТ» = НАЙДЕННАЯ УЯЗВИМОСТЬ

В чате с оператором слово **результат** (и синонимы: нашли, итог, вывод охоты) значит **только CONFIRMED уязвимость** с импактом по Rule 0.

Не результат:
- процесс проверки («прогнали SSTI / IDOR»)
- число попыток («70 проверок», «159 attempts»)
- вердикт покрытия EXHAUSTED / KILL / not-applicable
- «следующий класс: …»

Coverage пишется в `evidence/` и brain, не выдаётся оператору как итог. После охотника без CONFIRMED: молча продолжить цикл или одна строка, зачем нужен человек (OTP, второй аккаунт). Не пересказывать матрицу.
