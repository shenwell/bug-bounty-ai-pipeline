...
12. **«Результат» = найденная уязвимость.** Process, attempt counts, EXHAUSTED/KILL are coverage — not a result to the operator. Hunting Rule 32.
...
Apply: On session start, read `MEMORY.md`, `test_accounts.md`, `session_state.md`, brain files, workspace `AGENTS.md`, and `.env`. Treat these as authoritative. If a value is stale, update memory — don't request a replay.
...
### Don't call coverage a "result" to the operator
Why: a past engagement autopilot narrated every hunter stop as a result ("SSTI EXHAUSTED, 64 attempts, next is RCE"). The operator corrected: a result is only a found vulnerability. Process and attempt counts are not results.
Apply: User-facing chat: speak "результат" only for CONFIRMED vulns (Rule 0 impact). Write EXHAUSTED/KILL/attempt N to evidence + brain. After a dry hunter: continue silently, or one line if a human gate is blocking (login, account B). Never lead with "мы сделали N проверок".

### Never use placeholder values when the real value is one file away
Why: Agents default to `researcher`, `tester`, `YOUR_USERNAME`, or a generic UA when the correct value is in `policy.md`, `scope.yaml`, or `AGENTS.md`. Placeholders in program-required fields are a policy violation — potentially invalidating the entire recon phase.
...
Why: Per-agent caches under `brain/<agent>/` are invisible to `/status` and future sessions. The orchestrator flies blind on subsequent hunts.
...
Apply: On first tool invocation in a new workspace, prefer `uv run python3` / `uv run pytest`. If `AGENTS.md` mentions `uv`, it's mandatory.
...
### Flag contradictions between workspace and suite AGENTS.md — don't silently pick one
...
Apply: On session start, if both workspace and suite `AGENTS.md` exist, diff the model/tool policy sections. If they disagree, state the conflict in one line and ask which rules for this session, or default to suite policy and note that workspace `AGENTS.md` needs a refresh.
