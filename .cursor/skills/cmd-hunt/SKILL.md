...
Phase 2 only — **never** run portfolio discover/build/recon dossier from this command.

```bash
uv run python tools/dossier_precondition.py <slug> .
```

If exit code ≠ 0: stop and tell operator to run `/portfolio build <slug>` first (Phase 1).

## OUT OF SCOPE (Phase 2)

Forbidden during `/hunt`:

- `portfolio discover`, `portfolio build`, `portfolio refresh`
- Standoff scraper / disclosed crawl for dossier assembly
- Any workflow whose primary output is `data/dossiers/<slug>/contract.json`

Allowed: read existing dossier artifacts (`hunt_plan.md`, `landscape.md`, `auth_accounts.md`) already seeded into engagement.

## Post-session (optional)

After a hunt session with summaries in `engagements/<slug>/hunt/`:
...
- `rules/hunting.md` — hunting rules (Rule 0 → Rule 32). Rule 32: operator-facing "result" = CONFIRMED vuln only, not attempt counts or EXHAUSTED.
- `rules/hunt-progression-ladder.md` — when auth blocked or stuck: identity pivots, mine tenant, API-first (always-on)
...
## Phase 0: Session Intent (mandatory before first HTTP when auth exists)

If `recon/traffic-exercised.json`, `recon/session.json`, or `recon/sessions/` exist:

```bash
uv run python3 tools/session_intent.py validate --target <target> --root .
```

If validation fails, write intent (max 2 classes):

```bash
uv run python3 tools/session_intent.py write --target <target> \
  --session-goal "<feature → impact>" \
  --slice <focus_slice_id> \
  --classes idor,business-logic \
  --crown-jewel-ref <id from threat-model> \
  --differential-axis "role-A vs role-B on GET /api/..."
```

Read resolved intent before dispatch: `uv run python3 tools/session_intent.py show --target <target>`

## Phase 1: Read Before Touching (mandatory)

1. Read scope.yaml — confirm target is in scope
2. Read brain: `uv run python3 tools/brain.py brief <target>` — what's tested, what's exhausted
3. Read hacktivity.md — what has been reported and paid for this program
4. `uv run python3 tools/scope_check.py <target>` — hard validation
5. Read threat model: `brain/threat-model/<slug>.md` — if missing/stale run `/threat-model <target>` first
6. Read focus areas: `brain/focus-areas/<slug>.json` — hunt within current slice when present
7. **Traffic-informed P1 list** (mandatory when auth context exists):
   ```bash
   uv run python3 tools/traffic_informed.py validate --root .
   uv run python3 tools/traffic_informed.py rank --endpoints-file recon/endpoints.txt --output ATTACK_SURFACE_RANKING.md
   ```
   **Signal-fuzz** (after traffic rank, before Phase 2 — no surface probe A-I in `/hunt`):
   ```bash
   ...
   ```
   Hunters receive **P1 endpoints from traffic-boosted rank** plus `recon/sibling-candidates.json` when present. Do not hunt recon-only ghosts when exercised traffic exists.
8. Cheatsheet + session preamble for every hunter dispatch:
   ```bash
   uv run python3 tools/cheatsheet.py render-for-dispatch --target <target>
   uv run python3 tools/session_bridge.py render-preamble --root .
   uv run python3 tools/session_bridge.py list-roles --root .
   ```
   For BAC/IDOR/PE: use `--role low-priv-a` / `--role low-priv-b` preambles when available.
...
**Dispatch order:** `selected_classes` from session-intent FIRST, then remaining ranked classes.
Every hunter prompt MUST include one differential pair from `rules/differential-oracles.md`.
...
When dispatching from a fuzz-signal (`repro_template` in `recon/fuzz-signals.json`), seed the matrix from the signal's `suggested_class` — but **baseline pair (benign + malicious control) remains the mandatory first step** before mutation matrix expansion.

Before declaring any endpoint "exhausted" for a vuln class, enforce every item:

1. Build a test matrix: `method × content-type × auth-state × encoding × parser-trick`. Seed with `uv run python3 tools/intel_engine.py matrix <class>`.
...
6. Persist exhaustion to brain with what matrix cells were covered and what remains: `uv run python3 tools/brain.py record <target> recon "coverage-<class>" "<cells tested / skipped / remaining>"`.
...
`uv run python3 tools/intel_engine.py record-outcome --vuln-class "<class>" --result <confirmed|killed|downgraded|partial> --attempts <N> --elapsed-minutes <M>`

### Chain-Pressure Check (after every hunter)

The `chain_pressure_hook.py` SubagentStop hook scans `findings.json`
after every agent run and writes `brain/chain-pending.md`
...
```bash
cat brain/chain-pending.md 2>/dev/null
```
...
### User-Proxy Check (after every hunter — automatic via hook)

`.cursor/hooks.json` fires on **subagentStop**: writes
`brain/persona/user-proxy-pending.md` and injects a `followup_message`.

After every hunter (and whenever the hook fires):

1. `cat brain/persona/user-proxy-pending.md`
2. Apply `.cursor/agents/user-proxy.md` — emit `USER_PROXY_DECISION` before next dispatch
3. Optional full pass: dispatch `user-proxy` agent (model: inherit) with last hunter summary

Refresh persona from past dialogues periodically:

```bash
uv run python3 tools/user_persona.py ingest --max-files 100
```

Record explicit corrections:

```bash
uv run python3 tools/user_persona.py record-feedback "<text>" --kind correction
```

## Phase 4: FINDING PIPELINE (mandatory on every hunter signal)

When a hunter reports a potential finding, **do not** write brain `confirmed` without the gate trail.
...
```
If witness `ok: false` → re-dispatch hunter with read-back requirement; **no** brain `confirmed`.

**0c. Evidence score (programmatic, before validator):**
```bash
uv run python3 tools/evidence_score.py --finding <path> --update-gate
```

**1. Validator — 7-Question Gate**
Dispatch `validator` agent (model: inherit) with finding JSON path only.
```bash
uv run python3 tools/finding_record.py gate-update --finding <path> --gate validator --payload '{"decision":"PASS"}'
```
- **KILL** → cheatsheet miss + brain exhausted. Stop.
- **CHAIN REQUIRED** → dispatch `chain-builder`, re-validate if chain found.
- **PASS** → continue.

**2. Browser Verification** (client-side classes only: XSS, DOM, postMessage, open redirect with token)
Dispatch `browser-verifier` with PoC + WAF context. Record gate or KILL on BROWSER_REJECTED.

**3. Devil's Advocate**
Dispatch `dast-devils-advocate`. Record `gates.devils_advocate`.

**4. Finding Judge**
Dispatch `finding-judge` (read-only). Record `gates.judge`.
- **CONFIRM** → brain marker: `json:evidence/<host>/findings/<id>.json` only
- **KILL/DOWNGRADE** → no atomic confirmed brain line

**5. Post-confirm sibling sweep** (witness ok + judge CONFIRM or validator PASS)
Dispatch `sibling-mapper` (20 min time-box). Each sibling hit → new finding_record.

**6. Chain** — run `/chain` or dispatch `chain-builder` before `/report`.

## Phase 5: Document Non-Findings

For dead ends only:
- `uv run python3 tools/brain.py record <target> exhausted "<what failed>" "<why>"`
- `uv run python3 tools/coverage_record.py` for class exhaustion

**Never** use brain `confirmed` without `json:evidence/.../findings/<id>.json` reference.

## Phase 6: The A→B Signal Method

When finding pipeline confirms bug A, immediately check the chain table. Run /chain to find B.
