...
- `--waves N`: Run N hunt waves; each wave targets 1–2 focus-area slices (see `brain/waves/<target>.json`)

ALL agents dispatched by this command MUST use  in the Task tool call.

**Operator chat (Rule 32):** a "result" to the user is only a CONFIRMED vulnerability. EXHAUSTED, KILL, not-applicable, and attempt counts are coverage — write them to evidence/brain, do not announce them as the hunt outcome. After a dry hunter, continue the loop (or one line if a human gate blocks: OTP, account B).

## Phase 0: Dossier precondition (mandatory)

Phase 2 only — autopilot **starts after** portfolio RECON (hunt_plan, surface.json, auth_accounts.md exist in dossier/engagement).

```bash
uv run python tools/dossier_precondition.py <slug> .
```

If exit code ≠ 0: stop — operator must complete Phase 1: `/portfolio build <slug>` or `/new` + seed from dossier.

## OUT OF SCOPE (Phase 2)

Forbidden during `/autopilot`:

- `portfolio discover`, `portfolio build`, `portfolio analyze`, `portfolio refresh`
- Standoff Playwright scraper, disclosed crawl, dossier recon agents
- Replacing or rebuilding `data/dossiers/<slug>/contract.json`

Post-session sync (summaries → dossier):
...
6. **Session intent (when auth context exists):** validate or write `brain/session-intent/<slug>.json` via `tools/session_intent.py` (see workspace AGENTS.md Session Mindset). Gate blocks completion without it when traffic/session exists.
7. **Threat model (mandatory):** read `brain/threat-model/<slug>.md` + `brain/focus-areas/<slug>.json`. If missing/stale (`uv run python3 tools/threat_model.py status <target>`) → `/threat-model <target>` before surface probe.
8. **Wave ledger:** if `--waves N` or `--resume`, read `brain/waves/<slug>.json` via `uv run python3 tools/wave_ledger.py status --target <target>`
9. If `--resume`: read brain for tested/exhausted/remaining targets, skip to HUNT LOOP
10. Capture run metadata for the pre-completion gate and stash it in `.autopilot-run.env`:
   ```bash
   {
...
| cloud-misconfig | `cloud-hunter` | No cloud assets in scope.yaml (AWS/GCP/Azure/K8s/S3/blob) AND no cloud hostnames in recon |
| mobile-api | `mobile-hunter` | No mobile package/app in scope.yaml |
| web3 | `web3-auditor` | No smart contract / blockchain assets in scope.yaml |
...
**Escalation criteria (method-confusion):**
- `GET` returns 403 but `POST`/`PUT`/`PATCH`/`DELETE` returns 200 on same path → idor-hunter + auth-tester
- `OPTIONS` reveals allowed methods not shown in UI → test each on object IDs
- `TRACE`/`CONNECT`/`PROPFIND` anomalies → record and test auth on those verbs only
- Uniform 405 on all non-GET paths → `not-applicable` with method-matrix citation
...
Full checklist: `rules/cache-deception.md`.
...
Decision tree: `rules/payloads.md` § Surface probe D decision tree.
...
## SIGNAL-FUZZ (mandatory after SURFACE PROBE, before HUNT LOOP step 17)

Cheap differential probes on P1 endpoints — read-only default. Export tier
`oracle+readback` only; `trace-only` stays in attempts JSONL (no rank boost).

```bash
uv run python3 tools/signal_fuzz.py validate --root .
uv run python3 tools/signal_fuzz.py build-corpus --root .
uv run python3 tools/signal_fuzz.py run --write-mode read-only --max-rps 1 --limit 50
uv run python3 tools/signal_fuzz.py export --output recon/fuzz-signals.json --target "$TARGET"
```

Skip endpoint only with brain: `recon-skip:signal-fuzz endpoint:<url> policy:<clause>` —
this is NOT class exhaustion. One fuzz-run per endpoint per wave; log
`fuzz-boost-applied:<endpoint>` when rank boost applied.

Flush (step 24): validate + scope_check + merge corpus refresh; optional config pairwise second pass.

## HUNT LOOP (for each P1 target)

**Wave mode (`--waves N`):** before step 17, for each wave:
```bash
uv run python3 tools/wave_ledger.py start --target <target> --max-waves <N>
```
Hunt only the current slice from `brain/focus-areas/<slug>.json`. Inject slice preamble:
`You own slice <slice_id> only — hosts: [...] — do not hunt outside this partition.`

After slice exhaustion:
```bash
uv run python3 tools/wave_ledger.py complete-slice --target <target> --slice-id <id> --classes "idor,oauth"
uv run python3 tools/autopilot_gate.py --target <target> --mode autonomous --wave-only --slice-id <id>
```
...
20. Build autonomous class hypotheses (MANDATORY):
    ...
    uv run python3 tools/intel_engine.py classes \
      ...
      --target <target> \
      --focus-areas brain/focus-areas/<slug>.json \
      --slice <current_slice_id> \
      --limit 8 \
    ...
    **Priority override:** dispatch `selected_classes` from session-intent BEFORE default sequence.
...
22. **Iterate the Canonical Hunter Class Set** (see EXHAUSTION CONTRACT — 26 classes). For
    ...
    a. **Session-intent selected_classes** — dispatch these first when not yet covered.
    a0. **Fuzz-signals** — dispatch hunter per `recon/fuzz-signals.json` entry
       (`signal_tier == oracle+readback`) with `repro_template` + `readback_curl_template`.
    b. Surface probe seeds — e.g., wildcard `Access-Control-Allow-Origin` →
    ...
    c. Dispatch specialized hunter agent (model: inherit) with:
       ...
       - **Differential oracle** (MANDATORY): one pair from `rules/differential-oracles.md`
         (auth vs unauth, role A vs B, parser A vs B, or before/after mutation + read-back)
...
23. **CHAIN-PRESSURE CHECK (every subagent completion):**
    ...
    `brain/chain-pending.md` if any feeder
    ...
    a. `cat brain/chain-pending.md 2>/dev/null` —
...
23.5 **USER-PROXY CHECK (every subagent completion — hook + orchestrator):**

    `.cursor/hooks.json` `subagentStop` writes `brain/persona/user-proxy-pending.md`
    and returns a follow-up to simulate the researcher's decision without blocking
    on human input in `--autonomous` mode.

    On each loop iteration after chain-pressure:
    a. Read `brain/persona/user-proxy-pending.md`
    b. Apply `.cursor/agents/user-proxy.md` OR dispatch `user-proxy` agent
       (model: inherit) when the stop was a major milestone (validator PASS,
       hunter EXHAUSTED, context checkpoint).
    c. Parse `USER_PROXY_DECISION.verdict`:
       - `CHAIN_NOW` → chain-builder before next hunter
       - `DEEPEN` → re-dispatch same class with Depth Engine constraints
       - `ROTATE` → next host or vuln class per brain queue
       - `ESCALATE_HUMAN` → checkpoint; do not spin on captcha/OTP/second-role (record in `brain/accounts/<slug>.json`, pause for human)
       - `CHECKPOINT` → brain flush + tell user exact `/resume` command
    d. Log proxy steering: `uv run python3 tools/journal.py log note <target> "user-proxy: <verdict> — <next_action>"`

    Persona maintenance (weekly or after preference shifts):
...
24. **FLUSH CYCLE (every 3 subagent completions):**
    ...
    e. Refresh class hypotheses with latest telemetry:
       ...
       uv run python3 tools/intel_engine.py classes \
         ...
         --output CLASS_HYPOTHESES.md
       ```
    f. Refresh class hypotheses with latest telemetry:
       ```bash
       uv run python3 tools/intel_engine.py classes \
         --tech-stack "<latest stack context>" \
         --target <target> \
         ...
         --output CLASS_HYPOTHESES.md
       ```
    g. Cheatsheet preamble for next dispatches:
       ```bash
       uv run python3 tools/cheatsheet.py render-for-dispatch --target <target>
       ```
    h. Context checkpoint — check context usage:
...
**0. Structured finding record (mandatory first step):**
```bash
uv run python3 tools/finding_record.py create \
  --target <target> --host <host> --vuln-class <class> \
  --title "<short title>" --exploit-curl '<curl>' \
  --readback-curl '<readback if any>' --marker '<delta marker>'
```
Save the returned `path` and `brain_line`.

**0b. Programmatic witness (before validator):**
```bash
    uv run python3 tools/finding_record.py gate-update --finding <path> --gate witness --witness
```
If witness `ok: false` → re-hunt or downgrade to potential; do not dispatch validator yet.

**0b2. Evidence score:**
```bash
uv run python3 tools/evidence_score.py --finding <path> --update-gate
```

24. **Gate 1: Validator — 7-Question Gate**
    Dispatch `validator` agent (model: inherit) with finding JSON path only (fresh context — no hunter transcript).

    Record result:
    ```bash
    uv run python3 tools/finding_record.py gate-update --finding <path> --gate validator --payload '{"decision":"PASS","q_failed":null}'
    uv run python3 tools/evidence_score.py --finding <path> --update-gate
    ```

    - If **KILL** → cheatsheet miss + brain update, skip to next hunter. Done.
...
26. **Gate 3: Devil's Advocate — Adversarial Severity Check**
    ...
    Record DA verdict in finding JSON:
    ```bash
    uv run python3 tools/finding_record.py gate-update --finding <path> --gate devils_advocate --payload '{"verdict":"SURVIVES"}'
    ```

**Gate 4: Finding Judge (fresh context, read-only)**
    Dispatch `finding-judge` agent with ONLY `finding.json` + policy excerpt.
    ```bash
    uv run python3 tools/finding_record.py gate-update --finding <path> --gate judge --payload '<judge JSON>'
    ```
    - **KILL** → brain `da-killed` equivalent + cheatsheet miss. Stop.
    - **DOWNGRADE** → use adjusted severity for reporting.
    - **CONFIRM** → brain line with `json:evidence/.../findings/<id>.json` + cheatsheet hit:
    ```bash
    uv run python3 tools/cheatsheet.py append --target <target> --kind hit --text "<one-line lesson>"
    ```
    **Post-confirm sibling sweep (20 min):** dispatch `sibling-mapper` with source finding JSON + traffic-exercised.json. Each sibling hit → new finding_record pipeline.
...
27. **Reporting Pipeline (finding survived all gates)**
    ...
    a. Build autonomous chain plan first:
       ...
       uv run python3 tools/intel_engine.py chain-plan \
         --capability-file brain/patterns/capability-graph.json \
    ...
    e. Before submit, run guards:
       ```bash
       uv run python3 tools/never_submit_filter.py --finding <path>
       uv run python3 tools/cvss_version_guard.py --vector "<cvss vector>"
       uv run python3 tools/cvss_calc.py "<cvss vector>"
       ```
    f. Dispatch `report-writer` agent (model: inherit) with:
    ...
    g. Dispatch `quality-check` agent (model: inherit) — must score >= 7
    ...
    h. Run evidence sufficiency score before finalizing report:
       ```bash
