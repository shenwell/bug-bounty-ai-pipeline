...
Look in evidence/ directory. Before submit, pack into one folder per `policy/submit.md` (bug bounty) or equivalent:
1. `leads/<lead>/manifest.json` lists paste + attachments.
2. `uv run python scripts/pack_submit.py data/dossiers/<Dossier> <LEAD>`.
3. Operator opens `leads/<lead>/attachments/` — Ctrl+A upload; paste from sibling `*-paste.md`.
4. `validate_submit_paste.py` on paste in that folder — exit 0.
UI screenshots: `rules/screenshots.md` + verify before pack.
If no submission package exists:
...
7. Optional locale: if `.cursor/rules/pentest-agents-report-style-ru.mdc` applies, write report body in Russian without bold formatting.
