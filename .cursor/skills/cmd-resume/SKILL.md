...
3. **Read chain-pending advisory**: `cat brain/chain-pending.md 2>/dev/null` — feeder findings from prior session waiting for chain dispatch. SURFACE THESE FIRST — they have proven dollar value if chained, vs untested surface which is speculative.
...
6. **Access analysis:** read dossier `auth_accounts.md`, open-loops; if **new session** with token → run ladder **§2 cabinet inventory** first; if missing role → §1/§4; structural → §7 Standoff draft
7. **Persona alignment:** `uv run python3 tools/user_persona.py brief` — researcher priority signals and vetoes for this session
8. Show what's been tested vs what remains
9. Suggest next techniques based on brain patterns + persona priority_signals
