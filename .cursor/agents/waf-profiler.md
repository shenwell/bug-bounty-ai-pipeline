...
## Session Bridge (mandatory when WAF blocks curl)

When challenge pages block vanilla `curl`, coordinate with `browser-stealth-agent` to pass the challenge, then export cookies:

```bash
uv run python3 tools/session_bridge.py write \
  --host <target-host> \
  --waf-vendor "<cloudflare|akamai|...>" \
  --user-agent "<browser UA from stealth session>" \
  --cookies-json '[{"name":"cf_clearance","value":"..."}]'
```

All injection hunters MUST receive `uv run python3 tools/session_bridge.py render-preamble --root .` in their dispatch preamble. Injection classes cannot be marked exhausted without `recon/session.json` when this profile shows WAF blocking.
