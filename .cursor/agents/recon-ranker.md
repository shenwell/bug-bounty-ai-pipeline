...
deeper judgment from recon + brain context. Tag each P1 endpoint with `focus_area: <slice_id>` when `brain/focus-areas/<target>.json` exists. If `recon/endpoints.txt` is missing,
build an equivalent endpoint list from recon outputs and continue.

## Inputs
Read from: recon/, scans/, js-analysis/, brain/
