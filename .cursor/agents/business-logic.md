...
### WORKFLOW DISRUPTION (mandatory)

For multi-step flows, test at least one HTTP-fuzzable or browser-only disruption axis from `rules/differential-oracles.md` §B.

```
DIFFERENTIAL ORACLE + WORKFLOW DISRUPTION (mandatory):
  Pair: [baseline flow vs disrupted flow]
  Disruption axis: [skip-step | double-submit | stale-session | degraded | back-refresh | logout-mid-flow | parallel-plane]
  Pass if: [concrete delta in data or persisted state — not status alone]
  Read-back: [independent session / plane confirms]
```

Browser-only axes (`back-refresh`, `logout-mid-flow`, `parallel-plane`) require browser MCP — do not claim PASS from curl-only probes.

Brain marker on exhaustion: `workflow-disruption:<path> axes:<comma-separated> evidence:<file.jsonl>`
