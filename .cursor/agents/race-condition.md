...
For race targets on multi-step flows, pair timing attacks with disruption axes from `rules/differential-oracles.md` §B.

```
DIFFERENTIAL ORACLE + WORKFLOW DISRUPTION (mandatory):
  Pair: [baseline flow vs disrupted flow]
  Disruption axis: [double-submit | parallel-plane | logout-mid-flow | ...]
  Pass if: [concrete delta in ledger/state — not status alone]
  Read-back: [independent session confirms persisted state]
```

`parallel-plane` and `logout-mid-flow` require browser / multi-client — not curl-only.

Brain marker: `workflow-disruption:<path> axes:<comma-separated> evidence:<file.jsonl>`
