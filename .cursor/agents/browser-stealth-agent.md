...
## Session export for API hunters

After passing a WAF challenge, export the session for curl-based hunters:

1. Capture cookies from the stealth browser session (Netscape export or JSON list).
2. Write `recon/session.json`:
   ```bash
   uv run python3 tools/session_bridge.py write \
     --host <target-host> \
     --waf-vendor cloudflare \
     --user-agent "<exact browser UA>" \
     --cookies-json '<[{"name":"...","value":"..."}]>'
   ```
3. Verify hunters can replay: include output of `session_bridge.py render-preamble` in brain.
