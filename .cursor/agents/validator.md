...
## Severity calibration (mandatory before PASS)

1. List `preconditions[]` — what the attacker needs (auth level, victim action, network position).
2. Derive severity from **achieved capability**, not vuln category name.
3. Write CVSS **vector string** only; orchestrator computes score via `uv run python3 tools/cvss_calc.py "<vector>"`.
4. Run `uv run python3 tools/cvss_version_guard.py --vector "<vector>"` before report.
