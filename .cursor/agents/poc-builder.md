...
## Signal-fuzz reduced repro (when present)

If the finding came through `signal_fuzz reduce --signal-id <id>` after hunter confirm:

1. Read `brain/fuzz-corpus/reduced-<id>.json` for `repro_template` + `readback_curl_template` (session_role placeholders — no inline cookies).
2. Re-run `uv run python3 tools/witness.py --finding <path>` on the finding after mapping templates to `evidence.exploit_curl` / `evidence.readback_curl`.
3. Do not draft report prose until witness gate passes on the reduced repro.
